...
******** NEXT Check supplement homework 2
	 ...
	 :END:
********* Notes
- With just a brief glance, the message about $i$ looks the same for all questions, so students will probably only read the first one.
- Submit without constant and then include constant. I get "This answer is equivalent to the one you just submitted."
- Q2: "Note: there is always more than one possibility"
****** Clocking
******* TODO Piazza
	:LOGBOOK:
