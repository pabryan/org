...
******* DONE Send midterm to front desk for printing
	- State "DONE"       from "NEXT"       [2013-04-23 Tue 09:13]
	:LOGBOOK:
...
******* NEXT Set quiz 
	...
	:END:
       	...
       	:END:
******* Questions
1. Why isn't $C_1 e^{r t} + C_2 e^{rt}$ the *general form* of a linear, constant coefficient, homogeneous equation where the characteristic equation has the repeated root $r$?
   a. The Wronsikian is non zero.
   a. There are infinitely many $C_1, C_2$ satisfying the initial conditions.
   a. There exists $y_0, y_0'$ where no $C_1, C_2$ can be chosen to satisfy the initial conditions.

1. How many equations does the method of Variations of Parameters produce for $u_1', u_2'$?
   a. 0
   a. 1
   a. 2
