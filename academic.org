...
\]
where $a(z)$ is tangent. Is the tangent plane at $x$ parallel to the tangent plane to $\sphere^n$

****** Laplacian
Wwe compute $\laplace s$ in the standard parametrisation (i.e. $s$ is a function on $M$ not on $\sphere^n$) which I think is something like
\[
\laplace s = \ip{\laplace V}{\nor} + \ip{V}{\laplace \nor} + \text{Tr} (X, Y \mapsto \ip{\conx_X \nor}{\conx_Y V} + \ip{\conx_Y \nor}{\conx_X V}).
\]
