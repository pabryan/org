...
- Janelle notes:
  ...
  - [ ] For the last bit in the second paragraph was it supposed to be for all forward times like (-\infty,0) not t\in (-\infty, t_1)?
...
******** NEXT Review existing paper to find what needs to done
	 :LOGBOOK:
...
- Harnack
- $\int F(A) \to 0$
- smooth backwards convergence to equator
...
**** DONE Make sure I have reference letters
     ...
     :END:
**** NEXT Write cover letter for jobs
