...
I could also define the support function to be the distance from the origin to the supporting hyperplane with normal vector \(\nu\). That is the support function is defined in terms of direction. The supporting hyperplane should be either the totally geodesic hypersurface or horosphere with normal the given vector. This needs some thought though. It makes sense for a unit vector by choosing supporting hyperplane to be the one through the point on \(\bdry{K}\) with normal \(\nu\). For a non-unit vector \(v\), we want some sort of homogeneity. Probably we want to scale the ambient space by \(\lambda = \abs{v}\) and then take calculate from the unit vector in direction \(v\) and the resulting scaled hypersurface. How does this generalise? Probably we need to follow geodesic rays of some sort and then worry about cut locus...
******* Laplacian
